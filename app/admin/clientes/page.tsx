...
import { updatePortalLeadAction } from "./actions";
...
function getSourceLabel(source: string | null, campaign: string | null) {
  if (campaign && source) return `${source} · ${campaign}`;
  return campaign || source || "Acesso direto ao portal";
}

export default async function ClientesPage() {
  await getAccessContext();

  const [leads, total, newLeads, visits, converted] = await Promise.all([
    prisma.portalLead.findMany({
      orderBy: { createdAt: "desc" },
      take: 250,
    }),
    prisma.portalLead.count(),
    prisma.portalLead.count({ where: { status: "NOVO" } }),
    prisma.portalLead.count({ where: { status: "VISITA_AGENDADA" } }),
    prisma.portalLead.count({ where: { status: "CONVERTIDO" } }),
  ]);

  const indicators = [
    { label: "Leads recebidos", value: total },
    { label: "Novos", value: newLeads },
    { label: "Visitas agendadas", value: visits },
    { label: "Negócios concluídos", value: converted },
  ];
    ...
    <main className="min-h-screen bg-[#050505] text-white">
      <div className="mx-auto max-w-[1500px] px-6 py-12 lg:px-10">
        <header className="border-b border-white/10 pb-8">
          ...
          <h1 className="mt-3 font-serif text-5xl">Clientes e leads</h1>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-zinc-400">
            Contatos autorizados pelo visitante, identificados pelo imóvel e
            pela origem da campanha.
          </p>
        </header>
        ...
        <section className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {indicators.map((indicator) => (
            ...
            >
              ...
              <p className="mt-3 font-serif text-4xl">{indicator.value}</p>
            </article>
          ))}
        </section>
        ...
        <section className="mt-10">
          {leads.length === 0 ? (
            <div className="border border-white/10 bg-[#0a0a0a] p-10 text-center text-zinc-400">
              Nenhum lead recebido pelo portal até o momento.
            </div>
          ) : (
                  ...
                  >
                    <div className="grid gap-7 xl:grid-cols-[1.1fr_1fr_1.35fr]">
                      <div>
                        ...
                        <h2 className="mt-4 font-serif text-3xl">{lead.name}</h2>
                        <a
                        ...
                        >
                          {formatPhone(lead.phone)} · abrir WhatsApp
                        </a>
                      </div>
                      ...
                      <div className="text-sm leading-7 text-zinc-400">
                        ...
                        <p className="mt-4 text-xs text-zinc-500">
                          Origem: {getSourceLabel(lead.utmSource, lead.utmCampaign)}
                        </p>
                        <p className="mt-1 text-xs text-zinc-600">
                          Autorização registrada em {formatDate(lead.consentedAt)}
                        </p>
                      </div>

                      <form action={updatePortalLeadAction}>
